...
export async function login(request: Request, env: Env): Promise<Response> {
  ...
  ).bind(username).first<{ id: string; username: 'Amir' | 'Nazi'; password_hash: string; password_salt: string; password_iterations: number }>();
  ...
  if (!safeEqual(derived, row.password_hash)) {
...
export async function changePassword(request: Request, env: Env): Promise<Response> {
  ...
  const row = await env.DB.prepare('SELECT password_hash,password_salt,password_iterations FROM users WHERE id=?')
    .bind(auth.user.id).first<{ password_hash: string; password_salt: string; password_iterations: number }>();
  if (!row) return errorJson('کاربر یافت نشد.', 404, 'not_found');
  const currentHash = await pbkdf2(current, row.password_salt, row.password_iterations);
  if (!safeEqual(currentHash, row.password_hash)) return errorJson('رمز فعلی نادرست است.', 403, 'wrong_password');

  const salt = randomToken(16);
  const iterations = 600000;
  const hash = await pbkdf2(next, salt, iterations);
  const now = Date.now();
